...
class IndexerTest(test_base.BaseTest, IndexerTestMixin):
  ...
  def test_resolved_imports(self):
    ...
    with file_utils.Tempdir() as d:
      ...
      d.create_file(file_utils.replace_seperator("x/y.pyi"), stub)
      d.create_file(file_utils.replace_seperator("a/b.pyi"), stub)
      d.create_file(file_utils.replace_seperator("p/q.pyi"), stub)
      d.create_file(file_utils.replace_seperator("u/v.pyi"), stub)
      options = config.Options.create(d["t.py"])
      options.tweak(pythonpath=[d.path], version=self.python_version)
      ix = indexer.process_file(options)
